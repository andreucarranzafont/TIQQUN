...
export default function PokerTableUI() {
  ...
  // actualitza # jugadors mantenint estat
  useEffect(() => {
    setPlayers((prev) => {
  ...
  const pot = useMemo(() => players.reduce((s, p) => s + (p.bet || 0), 0), [players]);
  const street = boardStreet(board);

  /* ==== Recomanació bàsica (dinàmica) ==== */
  useEffect(() => {
    ...
    const toCall = Math.max(
      0,
      Math.max(...players.map((p) => p.bet || 0)) - (hero.bet || 0)
    );
    ...
    if (street === "PREFLOP") {
      if (toCall === 0) {
        ...
        const posFactor =
          myRole === "BTN" ? 2.2 : myRole === "SB" || "BB" ? 2.5 : 2.3;
      ...
      const anyBet = Math.max(...players.map((p) => p.bet || 0)) > (hero.bet || 0);
  ...
  function confirmAction(id) {
    ...
    a const amt = Number((amounts[id] || "0").toString().replace(",", "."));
    if (!p) return;
  ...
  function endDrag() {
    ...
    if (!d || !snap) return; // amb Snap OFF pots deixar-los fora del cercle
    setPlayers((ps) =>
      ps.map((p) => {
    ...
    <div className="w-full h-screen text-white">
      ...
      <div className="fixed top-0 left-0 right-0 z-30 bg-black/60 backdrop-blur px-3 py-2 flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          Radi
          <input type="range" min="160" max="380" value={radius}
      ...
      {/* chip/avís superior amb la recomanació (clar) */}
      <div className="fixed z-30 left-2 top-[46px]">
        <span className="inline-flex items-center gap-2 rounded-full bg-yellow-200 text-emerald-900 px-3 py-1 text-sm font-semibold shadow">
          <span>💡</span>{reco}
        </span>
      </div>
      ...
      <div className="pt-12 h-full">
        ...
        {/* FRANJA CLARA DE RECOMANACIÓ + ENTRADA RÀPIDA */}
        <div className="fixed left-0 right-0 bottom-0 z-30 bg-amber-50/95 text-black border-t border-emerald-700/30 shadow-[0_-8px_24px_rgba(0,0,0,0.25)]">
          <div className="max-w-6xl mx-auto px-3 py-3" aria-live="polite">
            <div className="text-emerald-700 text-sm font-semibold">QUÈ FER ARA</div>
