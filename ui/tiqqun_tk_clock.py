# -*- coding: utf-8 -*-
# TIQQUN TK Clock — amb Projectes, Logs, Reset/End i Panell de Rivals (heurístic)
...
class App(tk.Tk):
    def __init__(self):
        ...
        # PANELL DE RIVALS (centre, fila 1)
        self.rivals = ttk.LabelFrame(main, text="Rivals actius (lectura heurística)", padding=6)
        self.rivals.grid(row=1, column=2, columnspan=3, sticky="nsew", padx=6, pady=6)
        self.rivals_text = tk.Text(self.rivals, height=7, font=FONT_MONO, wrap="word")
        self.rivals_text.pack(fill="both", expand=True)
    ...
    def reset_all(self):
        ...
        self.rivals_text.delete("1.0", "end")
        self.rivals_text.insert("end", "—\n")
    ...
    def recompute(self, context: str = ""):
        ...
        # Rivals actius (heurística simple)
        others = [("P1",P1),("P2",P2),("P3",P3),("P5",P5),("P6",P6),("P7",P7),("P8",P8)]
        self.update_rivals_panel(others, board, pot, PSTATE["street"], mx)
    ...
    # ===== Panell Rivals: textura + inferència simple =====
    def board_texture(self, board: List[str]) -> Dict[str, bool]:
        tex = {"paired": False, "two_tone": False, "mono": False, "straighty": False, "flush3": False, "flush4": False}
        if not board: return tex
        suits = [self._suit(c) for c in board if c]
        from collections import Counter
        cs = Counter(suits)
        mx = max(cs.values()) if cs else 0
        tex["mono"] = (mx >= 5)
        tex["flush4"] = (mx == 4)
        tex["flush3"] = (mx == 3)
        # paired
        ranks = [self._rank_val(c) for c in board if c]
        tex["paired"] = (len(set(ranks)) < len(ranks))
        # straighty (molta connexió)
        ranks = sorted(set(ranks))
        if 14 in ranks: ranks = sorted(set(ranks + [1]))
        for i in range(len(ranks)-3):
            if ranks[i+3]-ranks[i] <= 4:
                tex["straighty"] = True
                break
        # two-tone (2 pals iguals al flop)
        if len(board) >= 3 and not tex["mono"] and not tex["flush4"]:
            tex["two_tone"] = any(v==2 for v in cs.values())
        return tex

    def infer_tags_for_player(self, p: Dict, pot: float, street: str, tex: Dict) -> List[str]:
        if not p.get("active", True) or p.get("act") == "fold":
            return ["— fora"]
        act = p.get("act",""); amt = float(p.get("amt",0.0))
        rel = (amt / pot) if pot > 0 else 0.0
        tags: List[str] = []
        # Acció bàsica
        if act in ("bet","raise"):
            if rel >= 0.75: tags += ["molt fort (overpair/dos parelles/set o shove)", "o combo-draw fort"]
            elif rel >= 0.50: tags += ["fort (top pair bon kicker / overpair / combo-draw)"]
            elif rel >= 0.25: tags += ["mitjà (top/second pair o draw)"]
            else: tags += ["probe/cbet petit (aire o draw)"]
        elif act == "call":
            tags += ["cap i cua (top/second pair, draw)"]
        elif act == "allin":
            tags += ["all-in (rang polaritzat)"]
        elif act == "":
            tags += ["check/pass"]

        # Textura del board
        if tex.get("flush4") or tex.get("mono"):
            tags += ["flush possible"]
        elif tex.get("flush3"):
            tags += ["flush draw possible"]
        if tex.get("straighty"):
            tags += ["escala possible"]
        if tex.get("paired"):
            tags += ["tauler aparellat (dos parelles/boath)"]

        # Afinar una mica per street
        if street == "flop":
            if act in ("bet","raise") and rel >= 0.5 and tex.get("paired"):
                tags += ["trio possible (set)"]
            if act in ("call","bet") and tex.get("flush3"):
                tags += ["FD freqüent"]
        elif street == "turn":
            if act in ("bet","raise") and tex.get("flush4"): tags += ["flush completat sovint"]
        elif street == "river":
            if act in ("bet","raise") and (tex.get("flush4") or tex.get("mono")):
                tags += ["representa flush"]
            if act in ("bet","raise") and tex.get("paired"): tags += ["boath possible"]

        # Neteja
        # agrupar i treure duplicats mantenint ordre
        seen=set(); tags=[t for t in tags if not (t in seen or seen.add(t))]
        return tags if tags else ["indefinit"]

    def update_rivals_panel(self, others: List, board: List[str], pot: float, street: str, mx: float):
        tex = self.board_texture(board)
        self.rivals_text.delete("1.0", "end")
        if not any(p.get("active", True) for _,p in others):
            self.rivals_text.insert("end", "—\n"); return
        for name, p in others:
            if not p.get("active", True): continue
            tags = self.infer_tags_for_player(p, pot, street, tex)
            pos = p.get("pos","None")
            act = p.get("act","") or "—"
            amt = p.get("amt",0.0)
            line = f"{name} [{pos}]  act={act} {amt if amt else ''}  →  " + "; ".join(tags) + "\n"
            self.rivals_text.insert("end", line)
