# -*- coding: utf-8 -*-
# ui/tiqqun_tk_clock.py (v6.3)
# Distribució:
#   - HERO centrat abaix (col centre)
#   - Esquerra (de baix a dalt): P1, P2, P3
#   - A dalt, al centre: P4 i P5 dins d'un contenidor centrat
#   - Dreta (de dalt cap avall): P6, P7, P8
#   - Centre: F1 F2 F3 T R (tauler ampli)
# Compacte: Acció i Quant petites. Confirm + "Actiu" a cada jugador.
...
class App(tk.Tk):
    def __init__(self):
        ...
        # ESQUERRA (de baix a dalt): P1, P2, P3
        self.p1 = PlayerFrame(main, "P1", 1)
        self.p1.grid(row=2, column=0, sticky="w", padx=4, pady=4)
        self.p2 = PlayerFrame(main, "P2", 2)
        self.p2.grid(row=1, column=0, sticky="w", padx=4, pady=4)
        self.p3 = PlayerFrame(main, "P3", 3)
        self.p3.grid(row=0, column=0, sticky="nw", padx=4, pady=4)

        # DALT CENTRE (P4/P5 dins contenidor centrat exactament a la columna 3)
        ...
        self.p4 = PlayerFrame(self.top_center, "P4", 4)
        self.p4.grid(row=0, column=0, sticky="e", padx=(0, 6), pady=0)
        self.p5 = PlayerFrame(self.top_center, "P5", 5)
        self.p5.grid(row=0, column=1, sticky="w", padx=(6, 0), pady=0)

        # DRETA (de dalt cap avall): P6, P7, P8
        self.p6 = PlayerFrame(main, "P6", 6)
        self.p6.grid(row=0, column=6, sticky="ne", padx=4, pady=4)
        self.p7 = PlayerFrame(main, "P7", 7)
        self.p7.grid(row=1, column=6, sticky="e",  padx=4, pady=4)
        self.p8 = PlayerFrame(main, "P8", 8)
        self.p8.grid(row=2, column=6, sticky="se", padx=4, pady=4)
        ...
        self.out = ttk.Label(bottom, text="RECOM —", font=FONT_MONO)
    ...
    def reset_all(self):
        ...
        for e in (self.center.f1, self.center.f2, self.center.f3, self.center.t, self.center.r):
            e.delete(0, "end")
        self.pH.pos.set("None")
        self.pH.act.set("")
        self.pH.amt.delete(0, "end")
        self.pH.active.set(True)
        self.pH.tag.config(text="-", style="Tag.TLabel")
        if hasattr(self.pH, "h1"):
            self.pH.h1.delete(0, "end")
        if hasattr(self.pH, "h2"):
            self.pH.h2.delete(0, "end")
