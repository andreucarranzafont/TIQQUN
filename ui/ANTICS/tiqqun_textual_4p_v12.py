...
# ui/tiqq u n_textual_4p_v12.py  (responsive 2x2 layout)
...
class TIQQUNTextual(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        # 2x2 responsive grid for players (fits smaller screens)
        with Grid(id="players", classes="row"):
    ...
    def action_recompute(self) -> None:
        ...
        # Street calc (explicit to avoid syntax issues)
        ...
        max_bet = max(p.bet for p in self.players.values())
        PSTATE['pot']=pot
        PSTATE['to_call_hero']=max(0.0, max_bet - hero_bet)
        PSTATE['hero_seat']=4

        tech = tech_eval(PSTATE['hero_cards'], PSTATE['board'], PSTATE['players'],
                         PSTATE['pot'], PSTATE['to_call_hero'], PSTATE['hero_seat'])
        flow = flow_score(PSTATE['hero_cards'], PSTATE['street'], PSTATE['board'])
        out = fuse_scores(tech, flow)
        self.query_one("#reclabel", Label).update(
            f"RECOM {out.decision} | conf={out.conf_final:.2f} | p_win={out.tech.p_win:.2f} | ev={out.tech.ev_hint:.2f} | flow={out.flow:.2f}"
        )
